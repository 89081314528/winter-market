...
public class CartService {
    ...
    public void mergeCart(String uuid, String username) {
        Cart generalCart = (Cart) redisTemplate.opsForValue().get(cartPrefix + uuid);
        Cart userCart = (Cart) redisTemplate.opsForValue().get(cartPrefix + username);
        List<CartItem> generalItems = generalCart.getItems();
        for (CartItem generalItem : generalItems) {
            userCart.getItems().add(generalItem);
        }
        generalCart.clear();
        redisTemplate.opsForValue().set(cartPrefix + uuid, generalCart);
