angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.createOrder = function () {
            $http.get('http://localhost:8189/winter/api/v1/orders/createOrder').then(function (response) {
                $scope.clearCart();
                $scope.loadCart();
                $scope.getOrders();
            });
    ...
    $scope.getOrders = function () {
            $http.get('http://localhost:8189/winter/api/v1/user/getOrders').then(function (response) {
